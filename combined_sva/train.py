...
def train_dual_encoder_probe(
    ...
    # -----------------------------------------
    # Freeze CLIP & DINO (already done in SVA_Model.__init__),
    # so only aggregator + classifier are trainable.
    # => Gather their params for the optimizer
    # -----------------------------------------
    params_to_train = list(model.sva.parameters()) + list(model.classifier.parameters())
