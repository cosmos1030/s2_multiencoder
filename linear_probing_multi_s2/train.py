...
def train_dual_encoder_probe(
    ...
    - epoch 최대 epochs
    - 반환값: 최적 모델이 저장된 checkpoint path (best_ckpt_path)
    """
    ...
    if resume_checkpoint and os.path.isfile(resume_checkpoint):
        ...
        if resume_checkpoint:
            print(f"[WARNING] Checkpoint not found: {resume_checkpoint}, starting from scratch.")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        # ----------------------
        # (3) Early Stopping 체크
        # Val Loss가 개선되면 체크포인트 저장, counter reset
        # 개선 안 되면 counter += 1
        # counter >= early_stopping_patience => break
        # ----------------------
        if val_loss_epoch < best_val_loss:
            ...
            # 모델 정보 저장 (예: 시간, epoch, 모델 이름 등)
            # 원하는 key:value를 자유롭게 넣어둘 수 있음.
            model_info = {
